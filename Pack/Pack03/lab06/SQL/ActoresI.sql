...
CREATE OR REPLACE PACKAGE  BODY PA_PERFIL IS
  ...
  PROCEDURE AD_CONTENIDO (xnombre IN VARCHAR, xurl IN VARCHAR, xtipo IN VARCHAR, xperfil IN VARCHAR, xduracion IN NUMBER, xidioma IN VARCHAR) IS
    ...
    END;
    PROCEDURE AD_TEMA(XNOMBRE IN VARCHAR,XPERFIL IN VARCHAR) IS 
    BEGIN
        INSERT INTO TEMA VALUES(XNOMBRE,XPERFIL);
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20121,'NO SE PUEDE AGREGAR EL TEMA');
    END;
    
    PROCEDURE EL_TEMA(XNOMBRE IN VARCHAR) IS 
    BEGIN
        DELETE FROM TEMA WHERE NOMBRE = XNOMBRE;
        COMMIT;
        EXCEPTION
        WHEN OTHERS THEN 
            ROLLBACK;
            RAISE_APPLICATION_ERROR(-20122,'NO SE PUEDE ELIMINAR EL TEMA');
    END;
